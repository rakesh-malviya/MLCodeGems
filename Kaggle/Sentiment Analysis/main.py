...
def RNN(x,seqlentf, weights, biases):
...
#     x = tf.stack(x, axis=0)
    ...
    outputs, states = tf.nn.dynamic_rnn(lstm_cell, x,sequence_length=seqlentf, dtype=tf.float32,time_major=True)
...
pred = RNN(x,seqlentf,weights, biases)
# print(pred)
# exit()
...
with tf.Session() as sess:
    ...
    while step * batch_size < training_iters:
        ...
        sess.run(optimizer, feed_dict={x: batch_x, y: batch_y,seqlentf:batch_seqlen})
        if step % display_step == 0:
            # Calculate batch accuracy
            acc = sess.run(accuracy, feed_dict={x: batch_x, y: batch_y,seqlentf:batch_seqlen})
            # Calculate batch loss
            loss = sess.run(cost, feed_dict={x: batch_x, y: batch_y,seqlentf:batch_seqlen})
            print("Iter" + str(step*batch_size) + ", Minibatch Loss= " + \
                  "{:.6f}".format(loss) + ", Training Accuracy= " + \
                  "{:.5f}".format(acc))
            
#             print("Testing Accuracy:", \
#                    sess.run(accuracy, feed_dict={x: X_test, y: y_test,seqlentf:seqlen_test}))
        step += 1
